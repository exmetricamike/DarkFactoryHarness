...
- **Never ask a question you can answer.** No `AskUserQuestion` while `/df-run` is active — record it in `project/DECISIONS.md`.
...
1. **Your test artifacts live outside the repos**, in `project/checks/`. Never dirty a repo's diff with your verification scripts.
...
6. **Update `project/BACKLOG.md` on every state transition**, immediately. It is the only source of truth for progress.
...
9. **`project/` is never committed.** It is per-project working state and git-ignored (only the scaffolding READMEs are tracked). When you commit in *this* repo, name the paths — never `git add -A`, never `-f` on `project/`.
...
- **Write `project/RESUME.md` after every completed WP.** One file write; caps the blast radius of an unannounced cutoff at one WP.
...
- Working-state file map under `project/`: `.claude/docs/file-map.md`
