...
It should be better at the tenth than at the first, and the only thing that carries across is `LESSONS.md` at the repo root — `project/` gets wiped or replaced, that file does not.
...
- **`/df-retro` in the morning does the distilling**, with the user's grades on `project/DECISIONS.md` in hand.
