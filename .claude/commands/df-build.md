---
description: Phase 4 - hand a frozen WP to the implementer, verify the result independently, commit
argument-hint: WP-XXX
---

# /df-build $ARGUMENTS — implement, verify, commit

Load the `implementer-protocol` skill; its step 0 resolves `roles.implementer` from `harness.config.json`.

**Precondition, hard:** WP state is `SPECCED`, the spec is `FROZEN`, and its log records a reviewer `VERDICT: READY` with no unresolved BLOCKER. Anything else → go run `/df-spec` and come back. This holds whoever implements it, the backend or you.
...
Resume the WP's implementer session — or start one when the implementer profile differs from the reviewer's, which is the normal case for a split configuration. Set BACKLOG state `BUILDING`. Expect minutes: use the profile's `limits.call_timeout_ms`, or background it.
Cross-repo WP: backend call first, then frontend, passing the *actual* implemented contract (paste the real route signatures/types the backend produced) into the frontend prompt. Never let the frontend guess.

If the call fails on quota/rate-limit/unreachable, load the `continuity` skill §B: record what the backend already wrote (`git status`/`git diff`), do not commit a half-finished WP, mark the WP `BLOCKED` with reason `backend-down`, and switch to the takeover ladder.
...
2. **Their tests**: run the repo's test + lint + typecheck commands from `PROJECT.md` yourself. The backend's `TESTS:` line is a claim, not evidence.
...
- **All pass** → Step 5.
- **Anything fails** → FIX round (up to the profile's `limits.fix_rounds`): exact command + exact output, no interpretation, no suggested patch unless the fix is unambiguous. Re-verify from Step 3 in full each time. After the last failed round → if the profile has a `fallback`, hand the WP to it on a fresh session with the spec, the current diff and the failure output, and record the switch in the log and the commit trailer. No fallback, or the fallback fails too → if the tree is worse than the baseline, revert this WP's commits (`git revert`, never `reset --hard` on committed work); mark `BLOCKED`, log the failure evidence, and **take the next WP** — unattended, you never halt the line for one package.
...
2. Append to `active-project/wps/WP-XXX.log.md`: implementer profile id, fix rounds, verification evidence, commit shas per repo, follow-ups discovered.
