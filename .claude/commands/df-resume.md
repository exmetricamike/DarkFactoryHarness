...
1. Read `project/RESUME.md`. If it is missing or says `Reason: none`, fall back to `/df-status` and stop — do not improvise a resume point.
...
7. Once you are actually moving again, replace `project/RESUME.md` with `Reason: none` so a stale checkpoint can never be resumed twice.
