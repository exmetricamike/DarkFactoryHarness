---
description: Phase 3 - write one WP spec, have Codex review it, resolve the issues, freeze it
...
- <bullet — kill the obvious scope creep before Codex finds it>

## 4. Design constraints
Decisions already made; Codex must not relitigate them.
...
## Step 3 — Codex spec review (mandatory — this is the gate)
...
|---|---|
| BLOCKER (contradicts the code, missing schema/contract, impossible order) | Check it against the repo. True → **amend the spec** and send the corrected version for another pass. False → reply in the next round with the file:line that disproves it and let Codex re-judge. Never leave a blocker standing. |
...
- Never accept a scope increase just because Codex offered it — but say why you declined.
